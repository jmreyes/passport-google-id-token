 ...
 * `googleId`, which should be set to `false` if the credentials are not valid.
 * If an exception occurred, `err` is set.
 *
 * Options:
...
GoogleTokenStrategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.error) {
    ...
    idToken = idToken || req.query.id_token || req.query.access_token || req.get('id_token') || req.get('access_token');
    ...
    self._verifyGoogleToken(parser, idToken, self._clientID, function(err, parsedToken) {
